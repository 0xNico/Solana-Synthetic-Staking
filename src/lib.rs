...
declare_id!("EYsDTppZPy3JRGTsmryLN5beLLC7D6MdG5Vpm4o1c5Sy");

#[program]
pub mod synthetic_stake {
    ...
    pub fn remove(ctx: Context<Operation>) -> Result<()> {
        ...
        if reciept.is_valid == 0 { // must have staked in order to remove
            return err!(ErrorCode::UserNotStaked)
        }
        ...
        // ~4 reward per second (note: unix time isnt always perfect) | roughly 12% APY.
        let reward_time = 40000*(curr_time - start_time);
        // compute burn amount after rewards for staking 
        let burn_amount = max(0, deposited_amount - reward_time as u64);
...
pub enum ErrorCode {
    ...
    #[msg("You must have a deposit receipt to remove.")]
    UserNotStaked,
}
